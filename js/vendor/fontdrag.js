...
var TCNDDF = TCNDDF || {};
(function() {
    var dropContainer, dropListing, displayContainer, domElements, fontPreviewFragment = document.createDocumentFragment(),
        styleSheet, fontFaceStyle, persistantStorage = window.localStorage || false,
        webappCache = window.applicationCache || window,
        contentStorageTimer;
    TCNDDF.setup = function() {
        dropListing = document.getElementById("fonts");
        dropContainer = document.getElementById("fontdrop_view");
        displayContainer = document.getElementById("fontdrop_view");
        styleSheet = document.styleSheets[1];
        dropListing.addEventListener("click", TCNDDF.handleFontChange, false); /* LocalStorage events */
        ...
        dropContainer.addEventListener("drop", TCNDDF.handleDrop, false); /* Offline event listeners */
    ...
    TCNDDF.handleDrop = function(evt) {
        ...
        for (var i = 0; i < count; i++) {
            var file = files[i],
                ...
                droppedFileName, droppedFileSize;
